      ...
      {items.map((item) => (
        <li key={item.id} className="flex items-center gap-2 rounded-lg border px-3 py-2">
          ...
          <Link
            href={item.href}
            prefetch
            className="hover:bg-muted/40 flex min-w-0 flex-1 items-baseline justify-between gap-4 rounded-md px-1 py-0.5 text-sm transition-colors"
          >
            <span className={`min-w-0 ${done[item.id] ? "text-muted-foreground line-through" : ""}`}>
              <span className="font-medium">{item.title}</span>
              {item.code ? (
                <span className="text-muted-foreground"> · {item.code}</span>
              ) : null}
            </span>
            <time dateTime={item.dueAt} className="text-muted-foreground shrink-0">
